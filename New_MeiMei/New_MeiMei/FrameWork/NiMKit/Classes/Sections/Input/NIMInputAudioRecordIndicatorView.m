...
- (instancetype)init {
    ...
    if(self) {
        ...
        _backgrounView = [[UIImageView alloc] initWithImage:[UIImage nim_imageInKit:@"icon_message_voice_start"]];
...
- (void)setPhase:(NIMAudioRecordPhase)phase {
    if(phase == AudioRecordPhaseStart) {
        _backgrounView.image = [UIImage nim_imageInKit:@"icon_message_voice_start"];
        //[self setRecordTime:0];
    } else if(phase == AudioRecordPhaseCancelling) {
        _backgrounView.image = [UIImage nim_imageInKit:@"icon_message_voice_cancle"];
